...
class SettingsWindow(Gtk.Window):
    ...
    def _load_config(self):
        ...
        self._button_toggled(
            self.radiobutton_download_days, self.spinbutton_download_days
        )
        self._button_toggled(
            self.radiobutton_upload_minutes, self.spinbutton_upload_minutes
        )
        self._button_toggled(self.radiobutton_upload_size, self.spinbutton_upload_size)
        self._button_toggled(
            self.checkbox_auto_status_change, self.checkbox_auto_status_change_if_scored
        )
    ...
    def _set_tracker_radio_buttons(self, radio_button=None):
        if (
            self.radio_tracker_local.get_active()
            or self.radio_tracker_mpris.get_active()
        ):
    ...
    def save_config(self):
        ...
        self.engine.set_config(
            "library_autoscan", self.checkbox_library_startup.get_active()
        )
        self.engine.set_config(
            "scan_whole_list", self.checkbox_library_entire_list.get_active()
        )
        self.engine.set_config(
            "library_full_path", self.checkbox_library_full_path.get_active()
        )
        ...
        self.engine.set_config("plex_ssl", self.checkbox_plex_ssl.get_active())
        self.engine.set_config(
            "plex_obey_update_wait_s", self.checkbox_plex_obey_wait.get_active()
        )
        ...
        self.engine.set_config(
            "jellyfin_port", str(int(self.spin_jellyfin_port.get_value()))
        )
        self.engine.set_config("jellyfin_user", self.entry_jellyfin_username.get_text())
        self.engine.set_config(
        ...
        self.engine.set_config(
            "tracker_update_wait_s", self.spin_tracker_update_wait.get_value_as_int()
        )
        self.engine.set_config(
            "tracker_update_close", self.checkbox_tracker_update_close.get_active()
        )
        self.engine.set_config(
            "tracker_update_prompt", self.checkbox_tracker_update_prompt.get_active()
        )
        self.engine.set_config(
        ...
        self.engine.set_config(
            "auto_status_change", self.checkbox_auto_status_change.get_active()
        )
        self.engine.set_config(
        ...
        """Update Colors"""
        self.config["colors"] = {
            key: reprColor(col.get_color()) for key, col in self._color_buttons.items()
        }
